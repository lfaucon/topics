...
If we can solve this problem efficiently, we can also solve an arbitrary LP efficiently, using the following: We consider the primal-dual LP, i.e. the LP that contains all the inequalities and equalities of the dual and the primal LP. Suppose, there are $k$ inequalities. At first, we test, whether the primal-dual LP is feasible, if not, there is no optimal solution to the LP by the duality theorem. Otherwise, we test, whether the LP becomes infeasible if we substitute for one of the inequalities itself with equality. If this makes the LP infeasible, that means, no feasible solution of the primal-dual takes the substituted inequality with equality, hence it is redundant and can be deleted. If the LP remains feasible that way, we make the substitution having now an LP with less inequalities. Iteratively, we obtain a maximization subject to a number of equality constraints. This can be solved with Gaussian Elimination. In the next section, we describe the Ellipsoid Method and related geometric propositions.
\section{The Ellipsoid Algorithm}
The following routine solves the Feasibility Problem:
\begin{algorithm}
\caption[a]{Ellipsoid Method}
\KwIn{Convex Set $P$, $\varepsilon > 0$}
\KwOut{Answer whether there is a }
...
$k \leftarrow k+1$\;}
\IfElse{center of $E_k$ in $P$}{\Return Yes\;}{\Return No\;
\end{algorithm}
